...
import { Car, DEFAULT_CAR, SHIELD_COLOR, SHIELD_DURATION_MS, type CarInput, type SurfaceFeel } from "../entities/Car";
...
interface ItemNoiseSample {
  ...
  r2: number;
  r3: number;
  // time.now snapshot at pickup. Patience is computed live (now - pickupAt ≥ aiPatienceFor)
...
export class RaceScene extends Phaser.Scene {
  ...
  private runRacing(dt: number, now: number) {
    ...
    for (const ai of this.ai) {
      ...
      if (aiActive && ai.items.length > 0 && ai.useItemAt != null && now >= ai.useItemAt) {
        if (this.shouldAiFire(ai, now)) {
          this.useItem(ai);
        } else {
          ai.useItemAt = now + AI_ITEM_RECHECK_MS;
        }
      }
    ...
    for (const p of this.pickups) {
      ...
      for (const c of this.cars) {
        if (Phaser.Math.Distance.Between(c.x, c.y, p.sprite.x, p.sprite.y) < 22) {
          ...
          if (!c.isPlayer) {
            // Noise sample frozen at pickup — see AISkillState.itemNoise comment.
            const skillState = this.aiSkill.get(c);
            skillState?.itemNoise.push(this.freshItemNoise(now));
            ...
            if (c.useItemAt == null) {
              const [lo, hi] = this.aiInitialDelayFor(c.items[0] as Item);
              c.useItemAt = now + Phaser.Math.Between(lo, hi);
            }
          }
          p.active = false;
  ...
  private useItem(car: Car) {
    ...
    car.useItemAt = null;
    if (!car.isPlayer) {
      const skillState = this.aiSkill.get(car);
      skillState?.itemNoise.shift();
      ...
      if (car.items.length > 0) {
        const [lo, hi] = this.aiInitialDelayFor(car.items[0] as Item);
        car.useItemAt = this.time.now + Phaser.Math.Between(lo, hi);
      }
    }
    ...
    switch (item) {
      ...
      case "shield":
        car.shielded = true;
        car.shieldExpiresAt = this.time.now + SHIELD_DURATION_MS;
  ...
  private freshItemNoise(now: number): ItemNoiseSample {
    return {
  ...
  private aiInitialDelayFor(item: Item): [number, number] {
  ...
  private aiPatienceFor(car: Car, item: Item): number {
  ...
  private aiNoiseFactor(car: Car): number {
    const skill = this.aiSkill.get(car)?.skill ?? 0.7;
    const inv = 1 - skill;
    return inv * inv;
  }
  ...
  private shouldAiFire(ai: Car, now: number): boolean {
    ...
    switch (item) {
      ...
      case "shield":  score = this.evalShieldUtility(ai, sample, noise); break;
    }
  ...
  // nonzero — score so the time-ramp + floor in shouldAiFire still lifts probability over patience.
  ...
  private evalShieldUtility(ai: Car, _s: ItemNoiseSample, _n: number): number | null {

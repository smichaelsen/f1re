...
### Surface textures (mow stripes, asphalt grain, gravel specks)
- Three procedural canvas textures generated once per game (guarded by `textures.exists`): `track-grass-stripes` (256px tile, two 128px bands, white half baked at full alpha), `track-asphalt-grain` (128px per-pixel black/white noise, alpha 0–36), `track-gravel-specks` (128px, ~150 sparse 2–3px black/white specks with wrapped edge copies so the tile seams invisibly).
- Rendered as world-bounds-sized `TileSprite` overlays at low alpha (`STRIPE_ALPHA=0.05`, `GRAIN_ALPHA=0.5`, `GRAVEL_ALPHA=0.5` — the texture's baked alpha does the rest). All overlays share origin `(wb.x, wb.y)` so tile patterns align across layers (outer-world stripes line up with infield stripes).
- **Paint order interleaves opaque fill layers with overlay sprites** so each overlay needs at most a single-polygon `GeometryMask` instead of exact region geometry: world grass → stripes (unmasked) → outside runoff fill + patches → gravel specks (masked to runoff outer loop — the asphalt/infield it also covers gets painted over next) → asphalt fill → grain (masked to asphalt outer loop) → inside runoff + patches → inside gravel specks (masked to asphalt inner loop) → infield grass → infield stripes (masked to inside runoff loop) → walls + edge lines + kerbs. `Track.draw()` got split from one `Graphics` into per-layer objects (`addLayer()`), all depth 0, ordered by creation.
- Gravel speck masks: whole band loop when the side's runoff surface is gravel (grass patches on a gravel band get specks too — accepted, specks are neutral black/white); gravel *patches* on a non-gravel band are masked individually.
- Mask graphics are created with `scene.make.graphics(..., false)` (never on the display list), so the two-camera ignore wiring in RaceScene/InspectScene is unaffected; the overlay sprites themselves are scene children created during `Track` construction and are picked up by both scenes' existing world-object collection.
- Grass runoff *bands* (the verge between asphalt edge and wall) stay flat-colored — only world grass + infield get mow stripes.
- Measured ~110 fps with 10 cars on Montmeló after the change (masks re-render per frame but are single polygons).
...
### Dust + sparks (game-feel particles)
- `src/entities/DustParticles.ts` — single Phaser `ParticleEmitter` (omnidirectional, lifespan 350–600 ms, brown 6×6 generated texture). `RaceScene.updateDust()` walks every car each frame; for the two rear OBB corners (`corners()` indices 0 and 3), it probes `track.surfaceAt` and emits one puff per off-asphalt corner whenever speed ≥ 60. Emission is per-frame so visual density follows car speed (more corners on grass + more frames per second on the surface = denser cloud), but particle lifespans are absolute so the cloud is bounded. Depth = 4 (above skid marks at 3, below pickups at 5).
- `src/entities/SparkParticles.ts` — two Phaser `ParticleEmitter`s sharing one 4×4 generated texture (tint palette `0xffe49a`/`0xffb24a`/`0xff7a1a`). The **burst** emitter (omnidirectional, lifespan 180–360 ms, speed 90–260) fires on actual impact: when the just-resolved wall hit produces a normal-velocity component `vn > 60`, `applyTrackBounds` explodes `min(20, round(vn/18))` particles at the worst-penetrating corner, nudged 2 px off the wall along `-normal` to avoid clipping. The **scrape** emitter (smaller/dimmer: scale 0.8, lifespan 100–240 ms, speed 30–130) drips one particle per contact frame while a car grinds along the wall below the burst threshold — gated on tangential speed `|vt| ≥ 140` so a slow lean against the wall stays silent. Both depth = 9.
- `src/entities/SmokeParticles.ts` — gray expanding puffs (lifespan 450–850 ms, scale grows 1.2 → 3.2, alpha 0.45 → 0, tints `0x8d8d8d`/`0xa6a6a6`/`0x6f6f6f`, 8×8 generated texture) for hard wall hits: `applyTrackBounds` emits `min(10, 4 + round((vn − 240)/50))` puffs on top of the spark burst when `vn > 240` (`SMOKE_VN_THRESHOLD`). Depth = 12 so the cloud envelops the car (cars 10, shield rings 11).
- All emitters are constructed during `RaceScene.create()` immediately after `createSkidMarks()` and before the `uiCam.ignore(worldObjects)` filter, so they auto-route to the world camera. Cleared on `SHUTDOWN`/`DESTROY`.

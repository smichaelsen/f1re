...
- `src/entities/SparkParticles.ts` — single Phaser `ParticleEmitter` (omnidirectional, lifespan 180–360 ms, tint palette `0xffe49a`/`0xffb24a`/`0xff7a1a`, 4×4 generated texture). `RaceScene.applyTrackBounds` triggers a burst on actual impact: when the just-resolved wall hit produces a normal-velocity component `vn > 60`, it explodes `min(20, round(vn/18))` particles at the worst-penetrating corner, nudged 2 px off the wall along `-normal` to avoid clipping. Wall-hugging contact (low `vn`) produces no sparks. Depth = 9 so they read as a hot flash above cars.
- Both emitters are constructed during `RaceScene.create()` immediately after `createSkidMarks()` and before the `uiCam.ignore(worldObjects)` filter, so they auto-route to the world camera. Cleared on `SHUTDOWN`/`DESTROY`.

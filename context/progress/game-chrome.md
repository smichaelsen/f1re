...
- **Rolling menu subtitle.** The line under the F1RE logo is picked at random per `MenuScene.create()` from `public/menu-subtitles.txt` (loaded once in `BootScene.preload` as a Phaser text asset, key `menuSubtitles`). The file is plain text, one subtitle per line; lines beginning with `#` and blank lines are skipped at render time so the file can be self-documenting. Adding new lines = edit the file, no rebuild. `MenuScene.pickSubtitle()` falls back to `2D Racing Fury` if the file is missing or empty.
...
- **Fastest-laps board.** Per-track all-time top-10 list (humans + AI mixed) persisted in `localStorage["f1re.fastestLaps"]` via `src/scenes/FastestLaps.ts`. `recordFastestLap(track, entry)` is called in `RaceScene.updateLapTracking` for every completed lap; the recorder sorts ascending by time, caps at 10 per track, and writes back. Loader validates each entry (positive ms, non-empty name, boolean isPlayer, finite recordedAt); bad entries are dropped silently. `MenuScene` exposes a third view `fastestLaps` (alongside `main` / `settings`) reachable from a `FASTEST LAPS` button that replaces the old in-button-row settings slot — the actual `SETTINGS` link moved to a small top-right text link with `setScrollFactor(0)`. The board view shows a 4-button track selector plus a 10-row monospace list; human entries render in `#ffd24a`, AI entries in `#ffffff`. Empty state shows a single line and hides the column header.
...
- **Two- or three-tone livery + design variant.** Each car has a `{ primary, secondary, variant, tertiary? }` livery (raw hex). Variants: `nose` (secondary fills the nose cone), `sidepods` (top + bottom edge stripes on the chassis), `spine` (centerline stripe nose → engine cover), `wingtips` (secondary on the front + rear wing stripes), `bull` (yellow nose + yellow front engine cover + red back engine cover, body stays primary — the only three-color variant, used by Rampage Racing to mirror the Red Bull RB livery). Other variants ignore `tertiary`.
- Textures generated lazily in `src/entities/CarSprite.ts`: `ensureCarTexture(scene, livery)` builds key `car_<primaryHex>_<secondaryHex>_<variant>[_<tertiaryHex>]` on first use and caches it. BootScene no longer pre-bakes per-color textures.

### Teams
- 11 teams in `src/entities/Team.ts`, each a 2026 F1 nod (`Scuderia Rosso`, `Silver Star`, `Rampage Racing`, `Papaya GP`, `Verde Sport`, `Alpha Bleu`, `Crown Royal`, `Forge Racing`, `Vorsprung Racing`, `Liberty Speed`, `Junior Bulls`). Each team owns `{ id, name, short, primary, secondary, tertiary?, drivers: [string, string], variant }`. `drivers` are two surname-style strings ≤ 8 chars each; the AI uses them directly so HUD/results show e.g. `WHITNEY`, `BAILEY` instead of the old 3-letter team code. `tertiary` is set only on Rampage (red, paired with the `bull` variant).
...
- **Camera world bounds are track-derived.** `Track.worldBounds` (centerline bbox + half-width + widest runoff + 600 pad) replaces the old fixed ±3000 rect for both the camera `setBounds` in `RaceCamera` and the grass background fill in `Track.draw()`. Montmeló spans x ±3500 and clamped against the fixed rect — the camera stopped panning before the track edge, making the far ends unplayable.
